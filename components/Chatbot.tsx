...
const Chatbot: React.FC = () => {
  ...
  return (
    <>
      ...
      {isOpen && (
        <div className="fixed bottom-24 right-6 w-full max-w-md h-[65vh] max-h-[500px] bg-white dark:bg-slate-800 border dark:border-slate-700 rounded-2xl shadow-2xl flex flex-col z-40 transform transition-all duration-300 origin-bottom-right">
          ...
          <div className="flex-1 p-4 overflow-y-auto bg-stone-50 dark:bg-slate-900">
            {messages.map((msg) => (
              <div key={msg.id} className={`flex my-2 ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`rounded-lg px-4 py-2 max-w-[80%] ${msg.sender === 'user' ? 'bg-teal-500 text-white' : 'bg-stone-200 dark:bg-slate-700 text-stone-800 dark:text-stone-200'}`}>
                  {msg.text}
                </div>
              </div>
            ))}
            {isLoading && (
              <div className="flex justify-start my-2">
                 <div className="rounded-lg px-4 py-2 bg-stone-200 dark:bg-slate-700 text-stone-500 dark:text-stone-400">
                    চিন্তা করছি...
                 </div>
              </div>
            )}
          ...
          <div className="border-t p-2 bg-white dark:bg-slate-800 dark:border-slate-700 flex items-center">
            <input
              type="text"
              value={input}
              ...
              className="flex-1 px-4 py-2 border rounded-full focus:outline-none focus:ring-2 focus:ring-teal-500 bg-white dark:bg-slate-700 text-stone-800 dark:text-stone-200 border-stone-300 dark:border-slate-600"
              disabled={isLoading}
            />
            <button
